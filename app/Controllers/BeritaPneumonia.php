...
class BeritaPneumonia extends Controller
{
    ...
    $beritapneumonia = $builder->findAll();

    // hitung manual (PALING AMAN & TIDAK ERROR CI4)
    $publish = 0;
    $draft = 0;

    foreach ($beritapneumonia as $b) {
    ...
    {
        $model = new BeritaPneumoniaModel();
        $beritapneumonia = $model->find($id);

        $newStatus = ($beritapneumonia['status_berita'] == 'publish') ? 'draft' : 'publish';
...
{
    $model = new \App\Models\BeritaPneumoniaModel();

    $data = $model->find($id);

    // 🔥 kalau data tidak ada
    if (!$data) {
        return redirect()->to('/beritapneumonia')->with('error', 'Data tidak ditemukan');
    }
    ...
    return redirect()->to('/beritapneumonia')->with('success', 'Data berhasil dihapus');
}
    ...
    public function publish()
{
    ...
    $beritapneumonia = $model->where('status_berita', 'publish')->findAll();

    $data = [
        'berita' => $beritapneumonia,
        'total' => count($beritapneumonia),
        'publish' => count($beritapneumonia),
        'draft' => 0,
        'status' => 'publish'
    ];
    ...
    public function draft()
{
    ...
    $beritapneumonia = $model->where('status_berita', 'draft')->findAll();

    $data = [
        'berita' => $beritapneumonia,
        'total' => count($beritapneumonia),
        'publish' => 0,
        'draft' => count($beritapneumonia),
        'status' => 'draft'
    ];
