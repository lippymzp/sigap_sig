...
<br>
    <section id="mapSection" class="container mt-5 mb-5 p-0">
        <h4 id="titleMap" class="text-dark mb-4 fw-bold">Peta Interaktif Persebaran dan Hasil Pemantauan</h4>
...
foreach($detailDesa as $key => $d){

    $kasus = (int)($d['jumlah_kasus'] ?? 0);
    $abj   = (float)($d['abj'] ?? 0);

    $kategori = strtolower(trim($d['kategori'] ?? ''));
    ...
    // desa kasus tertinggi
    if($kasus > $maxKasus){

        $maxKasus = $kasus;

        $desaTertinggi = $namaDesa;
    }
...
// rata-rata kasus dibulatkan
$rataKasus = count($detailDesa) > 0
    ? ceil($totalKasusRingkasan / count($detailDesa))
    : 0;

// rata-rata ABJ dibulatkan
$rataABJ = $jumlahABJ > 0
    ? ceil($totalABJ / $jumlahABJ)
    : 0;
    ...
    <div class="ringkasan-card">
        <div class="ringkasan-left">
            ...
            <p>
                ...
                <span><?= esc((string)$desaTertinggi) ?> </span>
...
let kasus = Number(d.jumlah_kasus || 0);
let penduduk = Number(d.jumlah_penduduk || 0);
let meninggal = Number(d.meninggal || 0);
let abj = Number(d.abj || 0);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    .toLowerCase();
        ...
        else if(kategori === "belum ada data"){
            warna = "#9ca3af";
        }
