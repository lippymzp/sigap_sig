...
<style>
    ...
    /* Tombol Filter Saat Aktif (Indikator Oranye) */
    ...
    .filter-body .form-input {
        background-color: #F4F6F8; border: 1px solid #F4F6F8; border-radius: 10px;
        padding: 12px 18px; width: 100%; font-size: 13px; color: #555; outline: none; appearance: none;
    }
    ...
    .btn-modal { padding: 10px 25px; border-radius: 25px; font-weight: bold; font-size: 14px; border: none; cursor: pointer; transition: 0.2s; }
    
    /* Pembaruan Warna Tombol Modal Sesuai Permintaan */
    ...
    .calendar-popup-fixed { 
        ...
        padding: 20px; transform: scale(0.95); transition: transform 0.3s ease;
    }
    ...
    .calendar-header-custom button { 
        background: #F4F6F8; border: none; border-radius: 50%; width: 32px; height: 32px; 
        display: flex; align-items: center; justify-content: center; cursor: pointer; 
        color: #333; font-weight: bold; font-size: 12px; transition: 0.2s; 
    }
    ...
    .calendar-table tr.week-row td:first-child { border-top-left-radius: 8px; border-bottom-left-radius: 8px; }
    .calendar-table tr.week-row td:last-child { border-top-right-radius: 8px; border-bottom-right-radius: 8px; }
    ...
    <div class="data-card">
        ...
        <div class="toolbar-container">
            <div class="toolbar-left">
                ...
                <!-- Tombol Filter ditambahkan class khusus 'btn-filter-icon' agar mudah dipilih oleh JS -->
                        ...
                        <tr class="data-row" 
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <a href="<?= base_url('dbd/hapus_pelaporan/'.$row['id_laporan']) ?>" class="btn-action btn-delete" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?');" title="Hapus"><i class="fa-solid fa-trash"></i></a>
                                </div>
                            </td>
                        </tr>
                    <?php 
        ...
        <!-- =====================================
             FOOTER PAGINATION DINAMIS JS
             ===================================== -->
        <div class="card-footer-custom">
            <div id="paginationInfo">Menampilkan hasil 0 dari 0 data</div>
            <ul class="pagination-custom" id="paginationControls">
                <!-- Tombol JS akan dibuat di sini -->
            </ul>
        </div>
...
<!-- =========================================
     MODAL KALENDER (DI TENGAH LAYAR)
     ========================================= -->
...
<script>
    ...
    let currentPage = 1;
    let tableRowsValid = []; // Menyimpan baris-baris yang lolos filter

    document.addEventListener("DOMContentLoaded", function() {
        document.getElementById('periodeYearDisplay').innerText = currentDataYear;
        updatePeriodeNavUI();
        applyMasterFilter(); // Jalankan filter pertama kali untuk menyesuaikan tahun default

        // Logika untuk menghilangkan alert sukses setelah 3 detik
        let alertBox = document.getElementById('alertSuccess');
        if (alertBox) {
            setTimeout(function() {
                alertBox.style.opacity = '0'; // Membuat elemen menjadi transparan secara perlahan
                setTimeout(function() {
                    alertBox.style.display = 'none'; // Menghilangkan elemen dari susunan tata letak
                }, 500); // Menunggu 0.5 detik hingga animasi transisi selesai
            }, 3000); // Waktu jeda 3 detik (3000 milidetik)
        }
    });


    /* =========================================
       1. LOGIKA MASTER FILTER & PENYORTIRAN
       ========================================= */
    function applyMasterFilter() {
        let table = document.getElementById("dataTable");
        let tbody = table.querySelector("tbody");
        
        // Ambil semua baris data asli (Abaikan baris pesan kosong)
        let allDataRows = Array.from(tbody.querySelectorAll("tr.data-row"));
        
        tableRowsValid = []; // Kosongkan daftar baris valid
        ...
        allDataRows.forEach(row => {
            ...
            // Jika lolos semua ujian filter, masukkan ke daftar valid
            if (matchSearch && matchYear && matchBulan && matchMinggu) {
        ...
        if (filterSortVal === "tertinggi") {
            ...
            // Jika tidak ada sort khusus, biarkan berurutan berdasarkan created_at dari database
            tableRowsValid.sort((a, b) => {
                let indexA = allDataRows.indexOf(a);
                let indexB = allDataRows.indexOf(b);
                return indexA - indexB;
            });
        }

        // 3. SUSUN ULANG DI DOM (HTML)
        tableRowsValid.forEach(row => tbody.appendChild(row));
        allDataRows.forEach(row => {
            if (!tableRowsValid.includes(row)) tbody.appendChild(row);
        });

        // 4. CEK APAKAH HASILNYA KOSONG
    ...
    /* =========================================
       2. LOGIKA PAGINATION (PER 10 DATA)
       ========================================= */
    function displayPage(page) {
        ...
        // Jika tidak ada data
        if (totalRows === 0) {
            document.getElementById('paginationInfo').innerText = "Menampilkan hasil 0 dari 0 data skrining";
        ...
        // Sembunyikan semua data valid
        for (let i = 0; i < totalRows; i++) {
            tableRowsValid[i].style.display = "none";
        }

        // Tampilkan hanya 10 data untuk halaman ini
        ...
        document.getElementById('paginationInfo').innerText = `Menampilkan hasil ${start + 1} sampai ${endDisplay} dari total ${totalRows} data skrining`;
    ...
    /* =========================================
       3. TRIGGER DARI HALAMAN
       ========================================= */
    // A. Saat Ketik di Pencarian
    function handleSearch() {
        filterSearchText = document.getElementById("searchInput").value.toLowerCase();
        applyMasterFilter();
    }

    // B. Saat Navigasi Tahun Diubah
    function changePeriodeYear(direction) {
        ...
        applyMasterFilter(); // Refresh tabel setelah tahun ganti
    }

    function updatePeriodeNavUI() {
        const nextBtn = document.getElementById('nextPeriodeBtn');
        if (currentDataYear >= currentRealYear) {
    ...
    // C. Saat Klik Tombol Modal (Terapkan / Reset)
    const modalOverlay = document.getElementById('modalFilterOverlay');
    function openFilterModal() { modalOverlay.classList.add('show'); }
    function closeFilterModal() { modalOverlay.classList.remove('show'); }
    modalOverlay.addEventListener('click', function(e) { if (e.target === this) closeFilterModal(); });
    ...
    function applyModalFilter() {
        ...
        // Logika untuk menyalakan indikator oranye pada tombol filter
        let isFilterActive = (filterBulanVal !== "" || filterMingguVal !== "" || filterSortVal !== "");
        let btnFilterIcon = document.querySelector('.btn-filter-icon');
        
        if (isFilterActive) {
            btnFilterIcon.classList.add('active-filter');
        } else {
            btnFilterIcon.classList.remove('active-filter');
        }
    ...
    function resetFilter() {
        ...
        // Matikan indikator oranye pada tombol filter
    ...
    /* =========================================
       4. KALENDER MINGGUAN KUSTOM (FILTER)
       ========================================= */
    ...
    let currentDateF = new Date();
    let activeMonthF = currentDateF.getMonth();
    let activeYearF = currentDateF.getFullYear();
    ...
    let prevBtnF = document.getElementById('prevMonthF');
    let nextBtnF = document.getElementById('nextMonthF');
        ...
        for (let i = 0; i < 6; i++) {
            ...
            for (let j = 0; j < 7; j++) {
                ...
                if (currentCellDate > todayLimitF) { cell.classList.add('disabled-day'); } 
                    ...
                    cell.onclick = function(e) { e.stopPropagation(); processWeekSelectionF(cellYear, cellMonth, cellDay); };
                }
